...
int main(int argc, const char **argv) {
  ...
  if (argc != 3 || (!is_format(argv[1]) || !is_format(argv[2]))) {
    std::cerr << "Must call diffcal with two arguments: ./diffcal <test_image_format> <reference_image_format>, where each format takes an integer, e.g. image%00d.png" << std::endl;
    exit(-1);
  }
